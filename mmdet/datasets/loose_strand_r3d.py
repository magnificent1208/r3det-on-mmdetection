...
class LOOSE_STRAND_R3D(CustomDataset):
    CLASSES = ('loose_strand',)

    def __init__(self, *args, **kwargs):
        super(LOOSE_STRAND_R3D, self).__init__(*args, **kwargs)

    def load_annotations(self, ann_folder):
        """
            Params:
                ann_folder: folder that contains r3d style annotations txt files
        """
            ...
            for ann_file in ann_files:
                ...
                with open(ann_file) as f:
                    ...
                    for si in s:
                        ...
                        x = sum(bbox[0::2]) / 4.0
                        y = sum(bbox[1::2]) / 4.0
                        w = float(bbox_info[2]) - float(bbox_info[0])
                        h = float(bbox_info[-3]) - float(bbox_info[1])
                        try:
                            a = float(bbox_info[-1])
                        except ValueError:
                            bbox_info[-1], bbox_info[-2] = bbox_info[-2], bbox_info[-1] 
                            a = float(bbox_info[-1])
